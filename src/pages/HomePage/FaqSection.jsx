...
const FaqSection = ({ questions = [], loading = false }) => {
  ...
  const commonQuestion = modifiedQuestions.slice(0, 6);
  const otherQuestion = modifiedQuestions.slice(6);
  return (
      ...
      <div className="container">
        <div className="faq__inner">
          ...
          <div className="faq__list">
            {!loading && (
              <Accordion data={commonQuestion} label="Thông tin chung" />
            )}
            {!loading && (
              <Accordion data={otherQuestion} label="Đăng ký, thanh toán" />
            )}
